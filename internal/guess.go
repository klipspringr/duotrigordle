...
// Response is a 10-bit number representing the game's response to a guess. The
// low 5 bits represent present (yellow), high 5 represent correct (green).
type Response uint16

// Five correct letters
...
func CheckGuess(guess, solution string) Response {
	...
	// if there's nothing in the pool, all letters must be Correct, so return
	if pool1 == 0 && pool2 == 0 {
		return AllCorrect
	}
